...
export class AuthController {
  ...
  constructor() {
    ...
    this.getApiCall = new GetApiCallService();
  ...
  #getTokenFromHeader(req) {
    ...
    if (
      tokenParts.length !== AUTH_CONTROLLER_CONSTS.MIN_TOKEN_PARTS ||
      tokenParts[AUTH_CONTROLLER_CONSTS.GET_AUTH_SCHEME_INDEX] !==
        AUTH_CONTROLLER_CONSTS.TYPE_OF_AUTH_SCHEME
    ) {
      return null;
    }
   ...
   * @param {*} req - request object
   * @param {*} res - response object
   * @returns API call count or error
   */
  addApiUsage = async (req, res) => {
    try {
      // Parse JSON body using the controller helper so this works with the custom HTTP server
      const { email } = await this.#readJson(req);

      if (!email) {
        return this.#sendJson(res, 400, { error: "Email is required" });
      }

      const result = await this.addApiCall.AddApiCallService({ email });
      ...
      if (error.message?.startsWith("NO USER WITH THE EMAIL")) {
  ...
  try {
    ...
    if (!userId) {
      return this.#sendJson(res, 400, { error: "ID is required" });
    }

    const { amount } = await this.getApiCall.getApiCallCount({ userId });
